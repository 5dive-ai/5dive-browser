...
PWVER=$(node -e 'process.stdout.write(require("'"$ROOT"'/browser/node_modules/playwright-core/package.json").version)' 2>/dev/null) \
  || { echo "playwright-core is not installed next to the plugin (npm install in browser) — the warm shape cannot exist without it" >&2; exit 69; }
